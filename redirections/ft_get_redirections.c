...
/*   Updated: 2019/04/02 08:50:42 by sid-bell         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int     ft_get_redirections(char **commands, t_command *command, int i, t_params *params)
{
	...
	if (!ft_isvalidred(commands[i]))
		return (-1);
	if (ft_strequ(commands[i], ">") || ft_strequ(commands[i], ">>") || ft_strequ(commands[i], "<<"))
	{
	...
	if (ft_strequ(commands[i], "<<"))
	{
		if (!commands[i + 1] || ft_is_token(commands[i + 1]))
			return (0);
		herdoc = ft_here_doc(commands[i + 1], params);
		command->herdoc = herdoc;
		return (i + 1);
	}
	...
	else if (ft_is_redirection(commands[i]) && commands[i + 1] && !ft_is_token(commands[i + 1]))
	{
		...
		else
		{
			if (open(commands[i + 1], ft_is_redirection(commands[i]) == 2 ? APPEND : TRUNC, 0777) < 0)
				ft_printf_fd(2, "21sh: p*ermission denied: %s\n", commands[i + 1]);
		}
		i += 1;
	}
	else
